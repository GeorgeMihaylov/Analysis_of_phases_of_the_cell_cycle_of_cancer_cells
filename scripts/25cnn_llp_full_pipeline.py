    ...
    for ip in imgs:
        ...
        try:
            if (not force_cpu) and (seg_model_gpu is not None):
                ...
                # Исправленный фрагмент кода (строки 340-350):
        except RuntimeError as e:
                msg = str(e).lower()
                if ("misaligned address" in msg) or ("cuda error" in msg):
                    logger.warning(
                        f"Cellpose GPU failed on {ip.name} ({e}). Retrying on CPU and switching to CPU for remaining images.")
                    force_cpu = True
                    try:
...
def predict_all_cells(cfg: Config, logger: logging.Logger, manifest: pd.DataFrame, model_path: Path) -> pd.DataFrame:
    ...
    # Объединяем с manifest по индексу (добавляем столбцы с предсказаниями)
...
def aggregate_condition_percentages(cfg: Config, logger: logging.Logger, pred_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(pred_df.columns) != len(set(pred_df.columns)):
        duplicates = pred_df.columns[pred_df.columns.duplicated()].tolist()
