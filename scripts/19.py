...
class EnhancedConfig:
    ...
    # Визуализация
    SAVE_SEGMENTATION_VISUALIZATION = True
    PLOT_COMPARISON_WITH_CYTOMETRY = True
    ...
    def __init__(self):
        for d in [self.RESULTS_DIR, self.MODELS_DIR, self.CELL_CROPS_DIR, self.VISUALIZATION_DIR]:
...
class VisualizationUtils:
    @staticmethod
    def save_segmentation_visualization(img: np.ndarray, masks: np.ndarray,
                                        cell_ids: List[int], fpath: Path,
                                        save_dir: Path = config.VISUALIZATION_DIR):
        """Сохраняет визуализацию сегментации с контурами клеток"""
        try:
            ...
            # Нормализуем изображение для лучшего отображения
            img_color = (img_color - img_color.min()) / (img_color.max() - img_color.min() + 1e-7) * 255
            img_color = img_color.astype(np.uint8)

            # Рисуем контуры каждой клетки
            for cell_id in cell_ids:
                mask = (masks == cell_id).astype(np.uint8)
                contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                # Используем случайный цвет для каждой клетки
                color = tuple(np.random.randint(0, 255, 3).tolist())
                cv2.drawContours(img_color, contours, -1, color, 2)

                # Подписываем ID клетки
                if len(contours) > 0:
                    M = cv2.moments(contours[0])
                    if M["m00"] != 0:
                        cX = int(M["m10"] / M["m00"])
                        cY = int(M["m01"] / M["m00"])
                        cv2.putText(img_color, str(cell_id), (cX, cY),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)

            # Сохраняем результат
            save_path = save_dir / f"{fpath.stem}_segmentation.png"
            cv2.imwrite(str(save_path), img_color)
            logger.info(f"Визуализация сегментации сохранена: {save_path}")

            # Также сохраняем маски отдельно
            masks_colored = np.zeros((masks.shape[0], masks.shape[1], 3), dtype=np.uint8)
            unique_masks = np.unique(masks)[1:]  # исключаем фон (0)
            for mask_id in unique_masks:
                mask_colored = np.random.randint(0, 255, 3)
                masks_colored[masks == mask_id] = mask_colored

            masks_path = save_dir / f"{fpath.stem}_masks.png"
            cv2.imwrite(str(masks_path), masks_colored)

            return save_path, masks_path

        except Exception as e:
            logger.error(f"Ошибка при сохранении визуализации: {e}")
            return None, None

    @staticmethod
    def plot_cytometry_comparison(df_results: pd.DataFrame,
                                  ground_truth_data: List[Dict] = config.GROUND_TRUTH_DATA,
                                  save_dir: Path = config.VISUALIZATION_DIR):
        """Создает графики сравнения с проточной цитометрией"""
        try:
            # Преобразуем Ground Truth данные в DataFrame
            gt_df = pd.DataFrame(ground_truth_data)

            # Подготовка данных из классификации
            results_pct = df_results.groupby(['genotype', 'dose', 'phase']).size().unstack(fill_value=0)
            results_pct = results_pct.div(results_pct.sum(axis=1), axis=0) * 100

            # Создаем отдельные графики для каждого генотипа и дозы
            unique_genotypes = df_results['genotype'].unique()
            unique_doses = df_results['dose'].unique()

            phases = ['SubG1', 'G1', 'G2M']
            colors = {'SubG1': 'red', 'G1': 'green', 'G2M': 'blue'}

            for genotype in unique_genotypes:
                for dose in unique_doses:
                    # Фильтруем данные
                    gt_filtered = gt_df[(gt_df['Genotype'] == genotype) & (gt_df['Dose'] == dose)]
                    pred_filtered = results_pct.loc[(genotype, dose)]

                    if gt_filtered.empty or pred_filtered.empty:
                        continue
                    ...
                    pred_values = []
                    for phase in phases:
                        gt_val = gt_filtered[gt_filtered['Phase'] == phase]['Value'].values
                        gt_values.append(gt_val[0] if len(gt_val) > 0 else 0)

                        pred_val = pred_filtered.get(phase, 0)
                        if isinstance(pred_val, (pd.Series, pd.DataFrame)):
                            pred_val = pred_val.iloc[0] if len(pred_val) > 0 else 0
                    ...
                    # Рисуем столбцы
                    bars1 = ax.bar(x - width / 2, gt_values, width,
                                   label='Проточная цитометрия',
                                   alpha=0.7, color=[colors[phase] for phase in phases])
                    bars2 = ax.bar(x + width / 2, pred_values, width,
                                   label='Наша классификация',
                                   alpha=0.7, color=[colors[phase] for phase in phases],
                                   hatch='//')
                    ...
                    # Настройки графика
                    ax.set_xlabel('Фаза клеточного цикла', fontsize=12)
                    ax.set_ylabel('Процент клеток (%)', fontsize=12)
                    ax.set_title(f'Сравнение с проточной цитометрией: {genotype}, Доза: {dose}Gy',
                    ...
                    # Сохраняем график
                    plt.tight_layout()
                    plot_path = save_dir / f"comparison_{genotype}_{dose}Gy.png"
                    plt.savefig(plot_path, dpi=300, bbox_inches='tight')
                    plt.close()

                    logger.info(f"График сравнения сохранен: {plot_path}")

                    # Также создаем таблицу сравнения
                    comparison_df = pd.DataFrame({
                        'Phase': phases,
                        'Cytometry': gt_values,
                        'Our_Prediction': pred_values,
                        'Difference': [abs(p - g) for p, g in zip(pred_values, gt_values)]
                    })

                    table_path = save_dir / f"comparison_table_{genotype}_{dose}Gy.csv"
                    comparison_df.to_csv(table_path, index=False)

                    # Логируем сравнение
                    logger.info(f"\nСравнение для {genotype}, Доза {dose}Gy:")
                    logger.info(
                        f"SubG1: Цитометрия={gt_values[0]:.1f}%, Предсказание={pred_values[0]:.1f}%, Разница={abs(pred_values[0] - gt_values[0]):.1f}%")
                    logger.info(
                        f"G1: Цитометрия={gt_values[1]:.1f}%, Предсказание={pred_values[1]:.1f}%, Разница={abs(pred_values[1] - gt_values[1]):.1f}%")
                    logger.info(
                        f"G2M: Цитометрия={gt_values[2]:.1f}%, Предсказание={pred_values[2]:.1f}%, Разница={abs(pred_values[2] - gt_values[2]):.1f}%")

        except Exception as e:
            logger.error(f"Ошибка при создании графиков сравнения: {e}")
...
class EnhancedImageProcessor:
    ...
    def process_file(self, fpath: Path) -> Optional[pd.DataFrame]:
        try:
            ...
            if config.SAVE_SEGMENTATION_VISUALIZATION:
                unique_ids = np.unique(masks)[1:]  # исключаем фон
                if len(unique_ids) > 0:
                    self.visualizer.save_segmentation_visualization(
                        img, masks, unique_ids, fpath
                    )
...
def main():
    ...
    # 7. Сравнение с проточной цитометрией
    if config.PLOT_COMPARISON_WITH_CYTOMETRY:
        logger.info("\n=== СРАВНЕНИЕ С ПРОТОЧНОЙ ЦИТОМЕТРИЕЙ ===")
        visualizer = VisualizationUtils()
        visualizer.plot_cytometry_comparison(df_all)

    logger.info(f"\nВсе результаты сохранены в: {config.RESULTS_DIR}")
    logger.info(f"Визуализации сохранены в: {config.VISUALIZATION_DIR}")
